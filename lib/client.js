    ...
    function VolumeButton () {
      ...
      React.useEffect(() => {
        ...
        const onMove = (event) => {
          ...
          state.pickHint = `朗读起点：第 ${hit.offset + 1} 字 · <span>${previewOf(hit.text, hit.local)}</span>`
          showHint(`<b>拖动选择朗读起点</b> · 第 ${hit.offset + 1} 字 · <span>${previewOf(hit.text, hit.local)}</span>`, state.rect)
        }
      ...
      const enterPick = () => {
        ...
        if (hit) setCursor(hit.offset, 'pick')
        else refreshCaret()
        if (resolved) {
          state.pickHint = `朗读起点：第 ${resolved.offset + 1} 字`
          showHint('<b>拖动选择朗读起点</b> · 松手定起点，再点图标朗读', state.rect)
        } else {
          showHint('<b>拖动选择朗读起点</b> · 松手定起点，再点图标朗读', null)
        }
        notify()
      }
