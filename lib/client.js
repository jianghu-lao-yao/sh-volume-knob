...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const HINT_PICK = '<b>松开后点击选起点，再松开即朗读</b>'
...
.sh-vk-hint b, .sh-vk-tip b { color: #ffd166; font-weight: 600 }
`
    ...
    // ---------------------------------------------------------------------
    /**
     * The icon's hover bubble. The native `title` attribute renders a system
     * tooltip that CSS cannot reach, so it stays as the accessible label while
     * this element does the visible work — with the same size and colour as the
     * drag hint.
     */
    let tipNode = null
    ...
    // ---------------------------------------------------------------------
    /**
     * Red used to mean "in-page volume is muted", which made the icon sit there
     * looking broken even while nothing was playing — and the flag survives a
     * reload, so it looked permanent. Red now means the one thing worth
     * shouting about: playback is running but this page is muted, i.e. you
     * pressed play and hear nothing.
     */
    const ICON_COLOR = (reading, silent) => (reading
      ? (silent ? '#e5484d' : 'var(--dsw-alias-brand-primary, #6f7bff)')
    ...
    function VolumeButton () {
      ...
      const silent = state.pageMuted || state.page === 0
      const hintText = () => {
        if (state.reading || state.busy) {
          return silent
            ? '停止朗读（页内音量已静音，听不到声音）'
            : '停止朗读'
        }
        return state.note || '点击朗读；上滑调音量；右拖选起点'
      }
      ...
      return React.createElement('button', {
        ...
        // text is shown in our own bubble with the drag hint's size and colour.
        onPointerEnter: () => showTip(hintText(), btnRef.current),
        ...
        style: {
          ...
          borderRadius: '8px',
          cursor: 'pointer',
          color: ICON_COLOR(state.reading || state.busy, silent),
          flex: '0 0 auto',
          touchAction: 'none',
          userSelect: 'none',
        },
      }, SpeakerIcon(state.reading || state.busy, silent && (state.reading || state.busy)))
    }
