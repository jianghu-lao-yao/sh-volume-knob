...
`
    ...
    function VolumeButton () {
      ...
      React.useEffect(() => {
        ...
        const onUp = () => {
          // Stage 3: releasing after the caret was placed starts the reading.
      ...
      const placePick = (x, y) => {
        ...
        if (!hit) return false
        setCursor(hit.offset, 'pick')
        state.pickStage = 'placed'
        state.pickHint = `朗读起点：第 ${hit.offset + 1} 字`
        showHint(`${HINT_PLACED} · 第 ${hit.offset + 1} 字 · <span>${previewOf(hit.text, hit.local)}</span>`, state.rect)
        notify()
        return true
      }
      ...
      const onPointerMove = (event) => {
        ...
        if (dx > DRAG_RIGHT_PX && dx >= Math.abs(dy)) current.rightDrag = true
      ...
      const onPointerUp = (event) => {
        ...
        if (current.rightDrag) { armPick(); return } // stage 1 done: wait for the click
